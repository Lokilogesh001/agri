...
def _infer_severity(predicted_class: str) -> str:
    if not predicted_class or predicted_class.lower()=="healthy": return "low"
    return "high" if any(k in predicted_class.lower() for k in _HIGH_SEVERITY_KEYWORDS) else "moderate"
@lru_cache(maxsize=1)
def _load_default_model():
    path=os.getenv("DISEASE_MODEL_PATH","ml/artifacts/best.pt")
    if not os.path.exists(path): raise RuntimeError(f"Disease model checkpoint not found: {path}")
...
def classify(image_path: str, model=None) -> dict:
    if not image_path: raise ValueError("image_path is required when disease_override is not provided")
    predictor=model or _load_default_model(); return predictor.predict(image_path)
def run(image_path: str|None=None, model=None, override: dict|None=None) -> DiseaseResult:
    ...
    predicted_class=result["predicted_class"]; confidence=float(result["confidence"])
