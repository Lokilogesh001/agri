...
def classify(image_path: str, model=None) -> dict:
    ...
    return (model or _load_default_model()).predict(image_path)
def run(image_path: str|None=None, model=None, override: dict|None=None) -> DiseaseResult:
    result=override if override is not None else classify(image_path,model=model); predicted_class=result["predicted_class"]; confidence=float(result["confidence"])
